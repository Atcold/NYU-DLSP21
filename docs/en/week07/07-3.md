...
$$
\vytilde= \text{Decoder}(\vh)
$$

Basically, $\vh$ is a squashing function $f$ of the rotation of our input $\vy$. ($\vy$ is the observation) and $\vytilde$ is a squashing function $g$ of the rotation of our hidden representation $\vh$.
...
Note that, here $\vy$ and $\vytilde$ both belong to the same input space, and $\vh$ belong to $\mathbb{R}^d$ which is the internal representation. $\mW{_h}$ and $\mW{_y}$ are matrices for rotation.
...
$$
\red{F}(\vy) = \red{C}(\vy,\vytilde) + R(\vh)
$$
