...
$$
\beta \red{D}_{\text{KL}}(\orange{\mathcal{N}}(\vmu, \vv) \mathrel{\Vert} \orange{\mathcal{N}}(\boldsymbol{0}, \boldsymbol{1})) = \frac{\beta}{2} \sum\limits_{i=1}^dv_i - \log{(v_i)} - 1 + \vmu_i^2
$$
...
The last term, $\vmu_i^2$, minimizes the distance between the bubbles and therefore prevents the "exploding" encouraged by the reconstruction term.
