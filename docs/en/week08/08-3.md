...
\vtheta_{\text{Enc}} = \vtheta_{\text{Enc}} - \triangledown_{\vtheta_{\text{Enc}}}\red{D}(\vzcheck,\vztilde)
$$
...
$$
\red{C}(\vy,\vytilde) = \Vert \vy - \vytilde \Vert^{2} =  \Vert{\vy - \text{Dec}[\text{Enc}(\vy)]}\Vert^{2}
$$
...
$$
\red{\tilde{F}}(\vy) = \red{C}(\vy, \vytilde) + \beta \red{D}_{\text{KL}}(\mathcal{N}(\vmu, \vv) \mathrel{\Vert} \mathcal{N}(\boldsymbol{0}, \boldsymbol{1}))
$$
...
The second term is the relative entropy (a measure of the distance between two distributions) between a Gaussian with mean $\vmu$, variance $\vmu$ and the standard normal distribution. If we expand this second term in the VAE loss function we get:

$$
\beta \red{D}_{\text{KL}}(\mathcal{N}(\vmu, \vv) \mathrel{\Vert} \mathcal{N}(\boldsymbol{0}, \boldsymbol{1})) = \frac{\beta}{2} \sum\limits_{i=1}^dv_i - \log{(v_i)} - 1 + \mu_i^2
$$
