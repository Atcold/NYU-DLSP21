...
As we know latent variables are useful in providing additional information that help in predicting multiple values of the target $\vy$, associated with the input $x$. We have seen earlier, in order to predict the target variable $\vy$ from latent variable $z$, the energy function is given as,

$$
\red{E}(\vy,\vz) = [\vy{_1} - g_1(\vz)]^2 + [y_2 - g_2(\vz)]^2, \vy \in \boldsymbol{Y}
$$
...
\boldsymbol{g}(z) = [w_{1}\cos(\vz), w_{2}\sin(\vz)]
$$
...
$$
\red{E}(\vy,\vz) = C(\vy,\vytilde) + R(\vz)
$$

where $\vytilde = \text{Dec}(\vz)$,
...
$$
\mathcal{L}(\red{F}_{\beta}(\cdot), \vect{Y})
$$
...
$$
\mathcal{L}(\red{F}_{\infty}(\cdot),\vect{Y})
$$
...
We then initialize $z$ to $\vztilde$, here we must ensure that these values are not too far apart from each other.

$$
\vzcheck = \underset{\vz}{\arg\min}{E(\vy,\vz)} \\
\red{E}(\vy,\vz) = \red{C}(\vy, \vytilde) + R(\vz) + D(\vz,\vztilde)
$$

We now minimise the loss functional in two steps, minimise $\mathcal{L}(\red{F}_{\infty}(\cdot), \vect{Y})$:

$$
\vtheta_{\text{Dec}} = \vtheta_{\text{Dec}} - \triangledown_{\vtheta_{\text{Dec}}}\red{C}(\vy,\vytilde)\\ \\
\vtheta_{\text{Enc}} = \vtheta_{\text{Enc}} - \triangledown_{\vtheta_{\text{Enc}}}D(\vzcheck,\vztilde)
$$
...
- Finally, the energy is the sum of the reconstruction and the regularization, $F(\vy) = C(\vy,\vytilde)+ R(\vh)$.
...
$$
C(\vy,\vytilde) = \Vert \vy - \vytilde \Vert^{2} =  \Vert{\vy - \text{Dec}[\text{Enc}(\vy)]}\Vert^{2}
$$

When the input is categorical we use cross entropy loss,

$$
C(\vy,\vytilde) = -\sum_{i = 1}^{n} \vy{_i} \log(\vytilde{_{i}}) + (1 - \vy{_{i}})\log(1 - \vytilde{_{i}})
$$
...
$$
\mathcal{L}(\red{F}(\cdot), \vect{Y}) = \frac{1}{m} \sum_{j = 1}^{m} \ell(\red{F}(\cdot),y^{(j)}) \in \mathbb{R} \\
...
As shown in *Fig. 4*, $\red{C}(\vy, \vytilde)$ penalises sensitivities to reconstructions along the manifold while $R(\vh)$ penalises how much the wiggling of $\vh$ comes from the wiggling of $\vy$. In this manner, you basically get to push up the energies only in the direction that are not used for the reconstruction.
...
For VAE:
- First, the encoder stage: we pass the input $\vy$ to the encoder. Instead of generating a hidden representation $\vh$ in AE, the hidden representation in VAE comprises two parts: $\boldsymbol{\mu}$ and $\vv$. And similar to using regularisation factor $R$ for $\vh$, we now use regularisation factors $U$ and $V$ for $\boldsymbol{\mu}$ and $\vv$, respectively.
- Next, we use a sampler to sample $\vz$ which is the latent random variable following a Gaussian distribution with $\boldsymbol{\mu}$ and $\vv$. Note that people use Gaussian distributions as the encoded distribution in practice, but other distributions can be used as well.
- Finally, $\vz$ is passed into the decoder to generate $\vytilde$.
    - The decoder will be a function from $\mathcal{Z}$ to $\mathbb{R}^{n}$: $\vz \mapsto \vytilde$.

In fact, for basic autoencoder, we can think of $\vh$ as just the vector $\boldsymbol{\mu}$ in the VAE formulation, with the variance set to zero. In short, the main difference between VAEs and AEs is that VAEs have a good latent space that enables generative process.
...
We can simply say that the new latent $\vz$ is going to be $\boldsymbol{\mu}$ which is the guess for the mean plus a epsilon that was sampled from a Gaussian whose amplitude is changed by the square root of the standard deviation $\vv$. So in this way, we can get gradients back flowing in these encoder.
...
$$
\red{\tilde{F}}(\vy) = \red{C}(\vy, \vytilde) + \beta D_{\text{KL}}(\mathcal{N}(\boldsymbol{\mu}, \vv) \mathrel{\Vert} \mathcal{N}(\boldsymbol{0}, \boldsymbol{1}))
$$

To visualize the purpose of each term in the free energy, we can think of each estimated $\vz$ value as a circle in $2d$ space, where the centre of the circle is $\boldsymbol{\mu}$ and the surrounding area are the possible values of $\vz$ determined by $\vv$.

If there is an overlap between any two estimates of $z$, (visually, if two bubbles overlap) this creates ambiguity for reconstruction because the points in the overlap can be mapped to both original inputs. The first term $C(\vy, \vytilde)$ forces to reconstructing these bubbles to the correct locations.
- The reconstruction loss $C(\vy, \vytilde)$ will push the points away from one another such there is no overlap.
- Another option for $C(\vy, \vytilde)$ to avoid overlap is to make the variance zero and then there are just points other than bubbles so there's no more overlap.
...
$$
\beta D_{\text{KL}}(\mathcal{N}(\boldsymbol{\mu}, \vv) \mathrel{\Vert} \mathcal{N}(\boldsymbol{0}, \boldsymbol{1})) = \frac{\beta}{2} \sum\limits_{i=1}^dv_i - \log{(v_i)} - 1 + \mu_i^2
$$
