...
<b>Figure 1:</b> Target propagation
</center>

Given an observation $\boldsymbol{y}$, we first compute $\tilde{z}$ which is the initial guess of the latent variable $z$
...
<b>Fig. 2</b>: Autoencoder
</center>

As shown in *Fig. 2*, to summarize at a high level, a very simple form of AE is as follows:
...
Denoising Autoencoder is a contrastive technique. *Fig. 3* shows the manifold of the denoising autoencoder and the intuition of how it works. 
...
Note that although VAE has "Autoencoders" (AE) in its name (because of structural or architectural similarity to auto-encoders), the formulations between VAEs and AEs are very different. See *Fig. 5* below.
...
For DAE as we mentioned before, the sampling was happening between <span style="color:#80b1d3">$\boldsymbol{y}$</span> and <span style="color:#fb8072">$\boldsymbol{\hat{y}}$</span>. So we move the input and then we decode everything into $\boldsymbol{\tilde{y}}$. For VAD we encode the input and we add noise in the hidden , so we just switch the position of the encoder in the sampler more or less.
...
$$
\beta D_{\text{KL}}(\mathcal{N}(\boldsymbol{\mu}, \boldsymbol{v}) \mathrel{\Vert} \mathcal{N}(\boldsymbol{0}, \boldsymbol{1})) = \frac{\beta}{2} \sum\limits_{i=1}^d(v_i - \log{(v_i)} - 1 + \mu_i^2)
$$
