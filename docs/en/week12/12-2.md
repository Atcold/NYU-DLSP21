...
$$\mathcal{D}=\left\{\left(\vect{x}_{1} \vect{y}\right)_{i}\right\}_{i=1, \ldots, N}$$
...
$$
\mathcal{L}^{D A E}(\vect{\theta})=-\log p(\vect{x} \mid \vect{x}+n)
$$
...
- repeat
  - decode $\vect{x}^s \sim \mathcal{M}^s$ to $\bar{\vect{y}}$ and create additional dataset $\mathcal{A}^s = \{(\vect{x}^s_j,\vect{y}_j\}_{j=1,\vect{...},M_s})$
...
- repeat
  - *Phase 1*
  - decode $\vect{y}^{t} \sim \mathcal{M}^{t}$ to $\bar{\vect{x}}$ with $p(\vect{x} \mid \vect{y})$, create additional dataset $$\mathcal{A}^{t}=\left\{\left(\bar{\vect{x}}_{k}, \vect{y}_{k}^{t}\right)\right\}_{k=1, \ldots, M_{t}}$$
  - decode $\vect{x}^{s} \sim \mathcal{M}^{s}$ to $\bar{\vect{y}}$ with $p(\vect{y} \mid \vect{x})$, create additional dataset $$\mathcal{A}^{s}=\left\{\left(\vect{x_{j}^{s}, \bar{\vect{y}}_{j}\right)\right\}_{j=1, \ldots, M}$$
  - *Phase 2*
