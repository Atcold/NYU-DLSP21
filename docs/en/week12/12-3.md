...
Here we discuss a state transition equation where $\vx$ represents the state, $\vect{u}$ represents control. We can formulate the state transition function in a continuous-time system where $\vx(t)$ is a function of continuous variable $t$.

<div class="MathJax_Display" style="text-align: center;">
$$
\begin{aligned}
\dot{\vx} &= f(\vx,\vect{u})\\
\frac{\partial \vx(t)}{\partial t} &= f(\vx(t), \vect{u}(t))
\end{aligned}
$$
</div>
...
In this example $\vx=(x,y,\theta,s)$ is the state, $\vect{u}=(\phi,\alpha)$ is the control.
...
$$
\vx[t]=\vx[t-1]+f(\vx[t-1], \vect{u}[t]) \mathrm{d} t
$$

To be clear, we show the units of $\vx, \vect{u}$.
...
### Define kinematic model of a tricycle $\dot{\vx}=f(\vx,\vect{u})$.
* $\vx$ represents state: ($x$, $y$, $θ$, $s$)
* $\vect{u}$ represents control: ($ϕ$, $a$)
* We feed $\vx[t-1]$ and $\vect{u}[t]$ to obtain the next state $\vx[t]$
