...
Below is an RNN schematic chart. We feed variable $x[t]$ and the previous state $h[t-1]$ into the predictor, while $h[0]$ is set to be zero vector. The predictor outputs the hidden representation $h[t]$.
<center>
