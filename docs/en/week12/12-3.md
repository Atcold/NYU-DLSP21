...
In optimal control (inference) shown as below, we feed the latent variable (control) $\vect{z}[t]$ and the previous state $\vect{x}[t-1]$ into the predictor, while $\vect{x}[0]$ is set to be $x_0$. The predictor outputs the state $\vect{x}[t]$.
