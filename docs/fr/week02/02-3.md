...
Nous vous invitons à suivre Alfredo Canziani [sur Twitter @alfcnz](https://twitter.com/alfcnz). Vous trouverez des vidéos et des manuels contenant des détails pertinents sur l'algèbre linéaire et la décomposition en valeurs singulières (SVD) en effectuant une recherche (en anglais) sur le Twitter d'Alfredo, par exemple en tapant `linear algebra (from:alfcnz)` dans le champ de recherche.
...
$$\mW_k \in \mathbb{R}^{n_{k} \times n_{k-1}}$$ représente la matrice d'une transformation affine correspondant $$k^{\text{ème}}$$ bloc et est décrite plus en détail ci-dessous.
...
Chaque image particulière peut donc être considérée comme un point dans un espace à 3 millions de dimensions.
...
Dans la partie suivante, nous visualisons comment un réseau neuronal sépare des points et quelques transformations linéaires et non linéaires. 
Vous pouvez y accéder [ici](https://atcold.github.io/pytorch-Deep-Learning/fr/week01/01-3/).
